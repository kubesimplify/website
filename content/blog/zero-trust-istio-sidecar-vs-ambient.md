...
draft: false
cover: /img/blog/zero-trust-istio-sidecar-vs-ambient/cover.jpg
tags: ["istio", "service-mesh", "zero-trust", "kubernetes", "ambient-mesh"]
---
...
```text
ZtunnelAccepted=UnsupportedValue ztunnel does not support HTTP attributes (found: methods, paths). In ambient mode you must use a waypoint proxy to enforce HTTP rules. Within an ALLOW policy, rules matching HTTP attributes are omitted. This will be more restrictive than requested.
```
