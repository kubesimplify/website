...
After I published the first version of this post, [EXO Labs posted results](https://x.com/alexocheema) from running 884 agent tasks across 100 models on a DGX Spark, and they found the same two things at scale: Qwen 3.6 35B A3B with MTP decodes faster than Lightning per token for most context lengths, and Qwen spends 45% more output tokens per task, so tasks take 42% longer overall. Nice to see one prompt's worth of my data and 884 tasks' worth of theirs point the same way. The full leaderboard is on [local.ai](https://local.ai).
