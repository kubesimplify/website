...
function loadAll() {
  ...
  const posts = files
    .map((file) => {
      ...
      // `authors:` (array) takes precedence over legacy single `author:`; first entry is primary
      const authorHandles = Array.isArray(data.authors) && data.authors.length > 0
        ? data.authors
        : [data.author];
      const authors = authorHandles.map(resolveAuthor);
      return {
        slug,
        ...
        // Opt in/out of the "new to these terms?" glossary line at the top of
        // the post. Undefined means "decide from tags" (see lib/glossary.js).
        glossary: typeof data.glossary === 'boolean' ? data.glossary : null,
        // Optional AEO block: frontmatter `faq:` list of {q, a} renders an
        // FAQ section + FAQPage JSON-LD (extractable by answer engines).
...
export function getAllAuthors() {
  ...
  for (const p of loadAll()) {
    for (const a of p.authors) {
      counts.set(a.handle, (counts.get(a.handle) || 0) + 1);
    }
  }
...
export function getPostsByAuthor(handle) {
  return loadAll().filter((p) => p.authors.some((a) => a.handle === handle));
}
