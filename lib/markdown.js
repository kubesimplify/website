  ...
  return (tree) => {
    function walk(node, parent, index) {
      if (
        ...
        node.children[0].value.trim().toLowerCase() === '{{llm-request-animation}}'
      ) {
        parent.children[index] = {
          type: 'paragraph',
          data: { hName: 'llm-request-animation' },
          children: [],
        };
        return;
      }
 ...
 *   {{llm-request-animation}}   →  interactive Day 1 LLM request visual
